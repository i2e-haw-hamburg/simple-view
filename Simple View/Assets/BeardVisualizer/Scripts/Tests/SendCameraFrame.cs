...
public class SendCameraFrame : MonoBehaviour
{
    ...
    [SerializeField] private RenderTexture cfgRenderTexture;

    [SerializeField] private string cfgMobileDevicePort = "12000";

    [SerializeField] private string cfgMobileDeviceAddress = "127.0.0.1";
        ...
        while (true)
        {
            ...
            if (connectedToMobileDevice)
            {
                ...
                NetworkAdapterFactory.GetUnityNetworkAdapterInstance()
                    .SendMessageOverTCP(screenMessage, IPAddress.Parse(cfgMobileDeviceAddress),
                        int.Parse(this.cfgMobileDevicePort));
            }
        }
    }
}