...
public class CodeBasedAssertionExample : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        var c = AssertionComponent.Create<FloatComparer>(out configurator, CheckMethod.Update | CheckMethod.Start,
            gameObject, "CodeBasedAssertionExample.FloatField", 3f);
        ...
        AssertionComponent.Create<ValueDoesNotChange>(CheckMethod.Update | CheckMethod.Start, gameObject,
            "CodeBasedAssertionExample.FloatField");

        // Validate the gameObject.transform.y is always equal to 3 (defined in this component)
        transform.position = new Vector3(0, 3, 0);
        AssertionComponent.Create<FloatComparer>(CheckMethod.Update, gameObject, "CodeBasedAssertionExample.FloatField",
            gameObject, "transform.position.y");

        // Check with the goReference field from this component is not set to null
        goReference = gameObject;
        var gc = AssertionComponent.Create<GeneralComparer>(CheckMethod.Update, gameObject,
            "CodeBasedAssertionExample.goReference", null);
